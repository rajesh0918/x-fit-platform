...
function ProductVisual({
  ...
  const selectedColorName = product.colors[selectedColor] || "Black";

  /*
    The current asset set contains one hero image per product.
    Until separate color photographs are added, we keep the real
    product photo and apply a subtle color treatment for the selected
    variant. This makes the color selector immediately functional
    without replacing the real product image.
  */
  const colorKey = selectedColorName.toLowerCase();

  const colorTreatment = (() => {
    if (colorKey.includes("red")) {
      return {
        overlay: "rgba(210, 35, 45, 0.42)",
        mixBlendMode: "color",
        filter: "saturate(1.18) contrast(1.04)",
      };
    }

    if (colorKey.includes("blue")) {
      return {
        overlay: "rgba(25, 85, 210, 0.42)",
        mixBlendMode: "color",
        filter: "saturate(1.12) contrast(1.04)",
      };
    }

    if (colorKey.includes("olive")) {
      return {
        overlay: "rgba(88, 101, 67, 0.42)",
        mixBlendMode: "color",
        filter: "saturate(0.9) contrast(1.03)",
      };
    }

    if (colorKey.includes("graphite")) {
      return {
        overlay: "rgba(92, 96, 101, 0.38)",
        mixBlendMode: "color",
        filter: "saturate(0.62) contrast(1.08)",
      };
    }

    if (colorKey.includes("white")) {
      return {
        overlay: "rgba(255, 255, 255, 0.30)",
        mixBlendMode: "screen",
        filter: "brightness(1.08) saturate(0.65) contrast(1.02)",
      };
    }

    if (colorKey.includes("lime")) {
      return {
        overlay: "rgba(204, 255, 0, 0.30)",
        mixBlendMode: "color",
  ...
  return (
    <div className="relative w-full h-full min-h-[520px] flex items-center justify-center">
      <img
        ...
        style={{
          filter: colorTreatment.filter,
        }}
      />

      {/* Dynamic color treatment */}
      {colorTreatment.overlay !== "transparent" && (
        <div
          className="pointer-events-none absolute inset-0 z-20 rounded-2xl"
          style={{
            background: colorTreatment.overlay,
            mixBlendMode: colorTreatment.mixBlendMode,
          }}
        />
      )}

      {/* Cinematic overlay */}
      <div className="pointer-events-none absolute inset-0 z-30 bg-gradient-to-t from-black/25 via-transparent to-transparent" />

      {/* Product telemetry */}
      <div className="absolute left-5 bottom-5 z-40 rounded-lg border border-white/10 bg-black/60 backdrop-blur-md px-3 py-2">
